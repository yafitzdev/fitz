...
def _aligned_units(contract: _QueryContract, units: list[EvidenceUnit]) -> list[EvidenceUnit]:
    ...
    if contract.identifiers or contract.phrase_anchors:
        return [
            unit
            for unit in units
            if _hard_anchor_score(contract, unit.text) > 0
            or _closure_bridge_score(contract, unit) > 0
        ]
...
def _read_result_unit(index: int, result: ReadResult, contract: _QueryContract) -> EvidenceUnit:
    ...
    content_text = _unit_text(result.content)
    return EvidenceUnit(
        index=index,
        kind=kind,
...
def _closure_bridge_score(contract: _QueryContract, unit: EvidenceUnit) -> int:
    """Score evidence retrieved through explicit closure bridge provenance."""
    ...
    if not isinstance(closure, dict):
        return 0
    score = 0
    identifiers = {str(value).lower() for value in closure.get("contract_identifiers", [])}
    phrase_anchors = {
        _normalize_text(str(value)) for value in closure.get("contract_phrase_anchors", [])
    }
    for identifier in contract.identifiers:
        if identifier.lower() in identifiers:
            score += 4
    for phrase in contract.phrase_anchors:
        if _normalize_text(phrase) in phrase_anchors:
            score += 3
    return score


def _best_unit(units: list[EvidenceUnit]) -> EvidenceUnit | None:
    """Return best unit by alignment and original rank."""
    if not units:
        return None
    return sorted(units, key=lambda item: (-item.alignment_score, item.index))[0]
